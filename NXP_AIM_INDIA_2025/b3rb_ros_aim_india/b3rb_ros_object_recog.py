...
class ObjectRecognizer(Node):
	...
	def __init__(self):
		...
		resource_name_yolo = "../../../../share/ament_index/resource_index/yolo11n_float32.tflite"
		...
		self.TARGET_CLASS_IDS = [2, 17, 22, 41, 46, 58, 77, 74]  # car, horse, zebra, cup, banana, potted plant, teddy bear, clock
	...
	def camera_image_callback(self, message):
		...
		for output in self.output_details:
			...
			x = np.transpose(x, (0, 2, 1)) # transpose to (batch, height, width, channels)
			# self.get_logger().info(f"Output tensor shape: {x.shape}, Sample values: {x[0, :5] if x.ndim == 3 else x[:5]}")
			if int8:
		...
		# Get model input size (e.g., 640x640)
		input_w, input_h = self.input_details[0]["shape"][2:0:-1]
		
		# The model output is (1, 8400, 84) after transpose
		# where 84 = 4 (box coords) + 80 (class scores for COCO dataset)
		output_data = y[0][0]  # Get the (8400, 84) numpy array of detections

		conf_thres = 0.25  # Confidence threshold
		iou_thres = 0.65   # IoU threshold for non-maximum suppression

		boxes = []
		scores = []
		class_ids = []

		# Iterate over all 8400 candidate detections
		for i in range(output_data.shape[0]):
			...
			# If the score is above the threshold, it's a valid detection
			if max_score > conf_thres:
				# Optional: Filter by your target classes
				if self.TARGET_CLASS_IDS is None or class_id in self.TARGET_CLASS_IDS:
					# Convert box from [center_x, center_y, width, height] normalized 
					# to [x1, y1, x2, y2] in pixel coordinates of the input image (e.g., 640x640)
		...
		if len(boxes) > 0:
			...
			indices = torchvision.ops.nms(boxes_tensor, scores_tensor, iou_thres)

			# Draw the final detections on the image
			for i in indices:
				box = boxes_tensor[i].numpy()
				start_point = (int(box[0]), int(box[1]))
				end_point = (int(box[2]), int(box[3]))

				conf = scores_tensor[i].item()
				cls_id = class_ids[i]
				object_name = self.label_names[int(cls_id)]

				# Update object count for the ROS message
				...
				cv2.putText(image, f"{object_name} {conf:.2f}",
							(start_point[0], start_point[1] - 10), 
							cv2.FONT_HERSHEY_SIMPLEX, 0.9, GREEN_COLOR, 2)

		# Resize the annotated image back to the original camera dimensions
		image = cv2.resize(image, (width, height))
		
		# Publish the debug image and the warehouse shelf message
